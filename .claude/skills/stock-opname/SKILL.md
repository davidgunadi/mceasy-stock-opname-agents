...
description: >
  ...
  IMEI Mismatch report. Invoke when the user wants to go from a raw Stock
  Quant export to a per-city Stock Opname reconciliation report (for a
  technician-level count instead, use /stock-opname-teknisi).
---
...
2. 🔎 **@stock-opname-comparator** — takes that cleaned CSV, matches it against the physical Stock Opname count by lot/serial number, and delivers a 3-sheet report (Summary, Product Summary, IMEI Mismatch).

Before we start, I need:
- The **raw Stock Quant CSV** export
- **City** (e.g. SBY, JKT)
- The **physical Stock Opname Excel file** for that city
- The **Inventory Masterfile Excel file** (price/category/group reference)"
...
2. Take the cleaned CSV path `@stock-quant-cleaner` produced (its stdout/summary names the output file — don't ask the user for it, and don't re-derive it yourself) and immediately invoke `@stock-opname-comparator` with that cleaned CSV, the city, the physical Stock Opname Excel file, and the Inventory Masterfile Excel file.
3. Let `@stock-opname-comparator` run `scripts/compare_stock_opname.py` and report its summary as it normally would.
