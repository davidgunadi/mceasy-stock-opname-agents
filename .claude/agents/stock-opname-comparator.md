...
description: >
  ...
  Opname count for the same city/date, and produces a 5-sheet Excel report
  (Summary, Product Summary, IMEI Mismatch, Aging Report, Aging Report
  Detail). Invoke when the user wants to reconcile ERP inventory against a
  physical stock count for a specific city.
model: sonnet
...
- **Product scope, price, and category come live from Odoo** (`scripts/odoo_client.py`), not from a local Inventory Masterfile file (replaced 2026-09-22). Scope = every Odoo product in `categ_id` 34, minus anything named in the user-provided exclusion workbook (single sheet, single "Product" column). Price = min `product.supplierinfo` price, currency-converted to IDR. Category / `New Group - July 25` = a price-tier (`C`/`B`/`A`) — **placeholder bands, explicitly confirmed changeable by the business owner**, see `scripts/odoo_client.py`'s `category_from_price`. Requires `ODOO_URL`/`ODOO_DB`/`ODOO_USERNAME`/`ODOO_API_KEY` in the repo's `.env` — if that's missing or the script errors on auth, tell the user rather than guessing at credentials.
...
- The `Aging Report` / `Aging Report Detail` sheets bucket the same scoped, non-abnormal ERP rows by whole-month age (from each lot's `Lot Created Date` to the opname date) into `< 3 Mo / 3-6 Mo / 6-9 Mo / 9-12 Mo / > 12 Mo`. `Aging Report` is Product × Aging bucket (summed qty); `Aging Report Detail` is one row per lot. A row with no parseable `Lot Created Date` is skipped from `Aging Report` but still listed in the detail sheet with a blank Age/Aging.

## Your Job

- Inputs from the user: **city** (e.g. "SBY", "JKT"), the **physical Stock Opname Excel file**, and the **exclusion workbook** (e.g. "Stock Opname Exclude Item.xlsx" — single sheet, single "Product" column of names to drop from scope).
...
- Run:
  ```
  python scripts/compare_stock_opname.py --cleaned-csv "<path>" --opname-xlsx "<path>" --exclusion-xlsx "<path>" --city <CITY>
  ```
...
Followed by the delivered `.xlsx` file (Summary, Product Summary, IMEI Mismatch, Aging Report, Aging Report Detail). If there are non-trivial mismatches, name the specific products (don't make the user open the file to find out what's wrong).
